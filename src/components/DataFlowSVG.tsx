    ...
    {/* Mobile version - top to bottom */}
      ...
      >
        ...
        <defs>
          ...
          <linearGradient id="mobileTextFade" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="white" stopOpacity="0" />
            <stop offset="20%" stopColor="white" stopOpacity="1" />
            <stop offset="80%" stopColor="white" stopOpacity="1" />
            <stop offset="100%" stopColor="white" stopOpacity="0" />
          </linearGradient>
          <mask id="mobileTextMask">
            <rect x="50" y="0" width="300" height="800" fill="url(#mobileTextFade)" />
          </mask>
        </defs>

        {/* Top - chaotic binary data streams */}
        <g className="opacity-80">
          <text x="50" y="80" fill="#22C55E" fontSize="10" fontFamily="monospace" opacity="0.6">
            {leftBinaryStrings[0]}
          </text>
          <text x="50" y="100" fill="#22C55E" fontSize="10" fontFamily="monospace" opacity="0.5">
            {leftBinaryStrings[1]}
          </text>
          <text x="50" y="120" fill="#22C55E" fontSize="10" fontFamily="monospace" opacity="0.7">
            {leftBinaryStrings[2]}
          </text>
          <text x="50" y="140" fill="#22C55E" fontSize="10" fontFamily="monospace" opacity="0.6">
            {leftBinaryStrings[3]}
          </text>
          <text x="50" y="160" fill="#22C55E" fontSize="10" fontFamily="monospace" opacity="0.5">
            {leftBinaryStrings[4]}
          </text>
        </g>

        {/* Converging lines from top to center with animated dots */}
        <g className="opacity-90">
          {[...Array(15)].map((_, i) => {
            const startY = 80 + i * 6;
            const startX = 50 + (i * 300) / 15;
            const midX = 200;
            const midY = 250;
            const endX = 200;
            const endY = 300;
            const opacity = 0.4 + (i % 5) * 0.1;
            const pathId = `mobile-path-${i}`;
            const animationDelay = (i % 10) * 0.3;
            const animationDuration = 3 + (i % 3);
        ...
        {/* Central organized binary stream */}
        <text
          x="200"
          y="350"
          fill="#22C55E"
          ...
          opacity="0.7"
          textAnchor="middle"
          mask="url(#mobileTextMask)"
        >
          {binaryString.substring(0, 20)}
        </text>

        {/* Output streams to platforms */}
        <g className="opacity-80">
          {[
            { label: 'CRM', x: 100 },
            { label: 'Finance', x: 150 },
            { label: 'Analytics', x: 250 },
            { label: 'Operations', x: 300 }
          ].map((platform, i) => {
            const startX = 200;
            const startY = 380;
            const endX = platform.x;
            const endY = 500 + i * 80;
            const outputPathId = `mobile-output-path-${i}`;
            const animationDelay = i * 0.5;
